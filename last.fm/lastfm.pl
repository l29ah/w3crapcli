...
$hash = md5_hex(md5_hex($passwd).$token);
@t = gmtime();
$req_str = "u=$login&s=$hash&a[0]=".StrEscaped($ARGV[0]).'&t[0]='.StrEscaped($ARGV[1]).'&b[0]='.StrEscaped($ARGV[2]).'&m[0]=&l[0]='.StrEscaped($ARGV[3]).'&i[0]='.StrEscaped(sprintf('%04d-%02d-%02d %02d:%02d:%02d', $t[5] + 1900, $t[4] + 1, @t[3, 2, 1, 0]));
